...
int MAX_SAMPLE_EVERY_CYCLE = 128;
...
void drawFrameTime(unsigned long us) {
    display.setCursor(4 * CHARWIDTH, 7 * CHARHEIGHT - 2); // almost at bottom, approximately centered
    displayln("%s ms %d/cycle", String(us * 0.001, 2).c_str(), sampleRate);
}
...
void loop() {
    ...
    if (pos >= NUMVALUES) {
        ...
        unsigned long totalSampleTime = (micros() - readStartTime) /
                                        2;     // Divide by 2 because we are taking twice as many samples as are shown on the screen
        ...
        pos = 0;
        count = 0;
        sampleRate = map(analogRead(A1), 0, 1023, 1, MAX_SAMPLE_EVERY_CYCLE);
    }
}
